...
class PaymentController extends Controller
{
    public function deposit()
    {
        ...
        // Get all enabled gateway currencies
        $allGatewayCurrencies = GatewayCurrency::whereHas('method', function ($gate) {
            $gate->where('status', Status::ENABLE);
        })->with('method')->orderby('name')->get();
        
        // Filter currencies based on user permissions
        $gatewayCurrency = collect();
        
        foreach ($allGatewayCurrencies as $currency) {
            $hasPermission = true; // Default to true if no specific permission exists
            $currencyPermission = $user->currencyPermissions()->where('gateway_currency_id', $currency->id)->first();
    ...
    public function depositInsert(Request $request)
    {
        ...
        // Check if user has permission to use this currency
        $hasPermission = true; // Default to true if no specific permission exists
        $currencyPermission = $user->currencyPermissions()->where('gateway_currency_id', $gate->id)->first();
