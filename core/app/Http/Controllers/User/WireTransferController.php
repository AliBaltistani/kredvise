...
class WireTransferController extends Controller {
    ...
    public function wireTransfer() {
        ...
        // Check if user has permission to use the default currency
        $user = auth()->user();
        ...
        if (!$hasPermission) {
            $notify[] = ['error', 'You do not have permission to use this currency for wire transfers'];
            return back()->withNotify($notify);
        }
    ...
    public function transferRequest(Request $request) {
        ...
        if (!$form) {
            $notify[] = ['error', 'Wire transfer form not found'];
            return back()->withNotify($notify);
        }
