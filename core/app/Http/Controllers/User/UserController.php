...
class UserController extends Controller {
    ...
    public function transactions() {
        ...
        // Get user's permitted currencies
        $permittedCurrencyIds = $user->activeCurrencyPermissions()->pluck('gateway_currency_id')->toArray();
        
        // If no specific permissions exist, show all transactions
        if (empty($permittedCurrencyIds)) {
            ...
            // Filter transactions based on currency permissions
            $filteredTransactions = collect();
            
            foreach ($allTransactions as $transaction) {
                // For deposit/withdrawal transactions, check the currency
                if ($transaction->remark == 'deposit' || $transaction->remark == 'withdraw') {
                    // Find the related deposit or withdrawal to check currency
                    if ($transaction->remark == 'deposit') {
                        $relatedRecord = Deposit::where('trx', $transaction->trx)->first();
                        if ($relatedRecord && in_array($relatedRecord->gateway_currency_id, $permittedCurrencyIds)) {
                            $filteredTransactions->push($transaction);
                        }
                    } else { // withdrawal
                        $relatedRecord = Withdrawal::where('trx', $transaction->trx)->first();
                        if ($relatedRecord) {
                            $method = WithdrawMethod::find($relatedRecord->method_id);
                            if ($method) {
                                $gatewayCurrency = GatewayCurrency::where('currency', $method->currency)->first();
                                if ($gatewayCurrency && in_array($gatewayCurrency->id, $permittedCurrencyIds)) {
                                    $filteredTransactions->push($transaction);
                                }
                            }
                        }
                ...
                // For transfer transactions, check the default currency
                elseif (strpos($transaction->remark, 'transfer') !== false) {
                    $gatewayCurrency = GatewayCurrency::where('currency', gs('cur_text'))->first();
                    if ($gatewayCurrency && in_array($gatewayCurrency->id, $permittedCurrencyIds)) {
                        $filteredTransactions->push($transaction);
                    }
                }
                // For other transactions, include them all
                else {
                    $filteredTransactions->push($transaction);
                }
            }
            ...
            $items = $filteredTransactions->slice($offset, $perPage)->values();
            $transactions = new LengthAwarePaginator($items, $count, $perPage, $page, [
                'path' => request()->url(),
                'query' => request()->query(),
            ]);
        }
